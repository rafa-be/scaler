...
void BinderSocket::close(CloseCallback onCloseCallback) noexcept
{
    if (_state == nullptr) {
        onCloseCallback();
    ...
    _state->_thread.executeThreadSafe([state = _state, onCloseCallback = std::move(onCloseCallback)]() mutable {
