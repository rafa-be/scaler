...
TEST_F(YMQAcceptServerTest, AcceptServer)
{
    ...
    auto onConnectionCallback = [&]([[maybe_unused]] scaler::ymq::internal::Client client) { ++nConnectionCount; };

    scaler::ymq::internal::AcceptServer server =
        scaler::ymq::internal::AcceptServer::init(loop, listenAddress, onConnectionCallback).value();
